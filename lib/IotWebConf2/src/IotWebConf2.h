...
const char IOTWEBCONF_HTML_HEAD[] PROGMEM =
    ...
    "d=document.documentElement.classList.toggle('dark');b.innerHTML=d?'&#9728;"
    "':'&#127769;';localStorage.setItem('tgs-theme',d?'dark':'light');};"
...
const char IOTWEBCONF_HTML_STYLE_INNER[] PROGMEM =
    ...
    "UI',sans-serif;--map-bg:#dbeafe;--map-land:#1d4ed8;--map-border:#93c5fd}"
    ...
    "var(--radius);padding:0.7rem "
    "1.5rem;font-size:1rem;font-weight:600;cursor:pointer;transition:opacity "
    ".2s,transform .1s;width:100%;letter-spacing:0.02em}"
    "button:hover,input[type=button]:hover{opacity:0.9;transform:translateY(-"
    "1px)}"
    ...
    ".G{color:var(--success);font-weight:600}.R{color:var(--danger);font-"
    "weight:600}"
    ".logo{transition:filter .3s}"
    "html.dark .logo{filter:invert(1) drop-shadow(0 0 15px "
    "rgba(59,130,246,0.45))}"
    "#tt{position:fixed;top:.75rem;right:.75rem;z-index:9999;width:auto!"
    "important;padding:.35rem "
    ".7rem;background:var(--surface2);color:var(--text2);border:1px solid "
    "var(--border);border-radius:var(--radius);font-size:1.05rem;cursor:"
    "pointer;line-height:1;transition:all .2s;}"
...
const char IOTWEBCONF_HTML_SCRIPT_INNER[] PROGMEM =
    ...
    "function pw(id){var "
    "x=document.getElementById(id);if(x.type==='password'){x.type='text';}else{"
    "x.type='password';}}";
const char IOTWEBCONF_HTML_HEAD_END[] PROGMEM = "</head><body>";
const char IOTWEBCONF_HTML_BODY_INNER[] PROGMEM =
    "<div "
    "style='text-align:left;display:inline-block;min-width:260px;max-width:"
    "580px;width:90%;padding:1.5rem 0;'>\n";
...
const char IOTWEBCONF_HTML_FORM_END[] PROGMEM =
    "<button type='submit' style='margin-top:1rem;'>Apply</button></form>\n";
const char IOTWEBCONF_HTML_SAVED[] PROGMEM =
    "<div style='background:rgba(34,197,94,0.12);border:1px solid "
    "var(--success);border-radius:var(--radius);padding:1rem;margin:1rem "
    "0;text-align:center;'>&#10003; Configuration saved<br/>Return to <a "
    "href='/'>home page</a>.</div>\n";
const char IOTWEBCONF_HTML_END[] PROGMEM = "</div></body></html>";
const char IOTWEBCONF_HTML_UPDATE[] PROGMEM =
    "<div style='padding-top:1.5rem;'><a href='{u}' "
    "style='color:var(--accent2);font-size:0.85rem;'>Firmware "
    "update</a></div>\n";
const char IOTWEBCONF_HTML_CONFIG_VER[] PROGMEM =
    "<div "
    "style='font-size:0.65rem;color:var(--text2);margin-top:0.5rem;'>Firmware "
    "config version '{v}'</div>\n";
...
{
public:
  WifiParameterGroup(const char* id, const char* label = NULL)
...
public:
  ...
  /**
   * Provide an Arduino pin here, that has a button connected to it with the
   * other end of the pin is connected to GND. The button pin is queried at for
   * input on boot time (init time). If the button was pressed, the thing will
   * enter AP mode with the initial password. Must be called before init()!
   ...
   *   - Rapid blinks, but mostly on - AP mode, waiting for configuration
   * changes.
   *   - Normal blinks - Connecting to WiFi.
   *   - Mostly off with rare rapid blinks - WiFi is connected performing normal
   * operation. User can also apply custom blinks. See blink() method! Must be
   * called before init()!
   *   @statusPin - An Arduino pin. Will be configured as OUTPUT!
   *   @statusOnLevel - Logic level of the On state of the status pin. Default
   * is LOW.
   */
  void setStatusPin(int statusPin, int statusOnLevel = LOW);

  /**
   * Add an UpdateServer instance to the system. The firmware update link will
   * appear on the config portal. The UpdateServer will be added to the
   * WebServer with the path provided here (or with "firmware", if none was
   * provided). Login user will be IOTWEBCONF_ADMIN_USER_NAME, password is the
   * password provided in the config portal. Should be called before init()!
   *   @updateServer - An uninitialized UpdateServer instance.
   *   @updatePath - (Optional) The path to set up the UpdateServer with. Will
   * be also used in the config portal.
   */
  ...
  /**
   * IotWebConf2 is a non-blocking, state controlled system. Therefor it should
  ...
  /**
   * Specify a callback method, that will be called upon WiFi connection
   * success. Should be called before init()!
   */
   ...
   * the "EEPROM" parallel to IotWebConf2, now this is the time to disable these
  ...
  /**
   * Specify your custom Access Point connection handler. Please use
   * IotWebConf2::connectAp() as reference when implementing your custom
   * solution.
   */
  ...
  /**
   * Specify your custom WiFi connection handler. Please use
   * IotWebConf2::connectWifi() as reference when implementing your custom
   * solution. Your method will be called when IotWebConf2 trying to establish
   ...
   * By default the handler implementation returns with NULL, as seen on
   * reference implementation IotWebConf2::handleConnectWifiFailure(). This
   * means we need to fall back to AP mode. If it method returns with a (new)
   * WiFi settings, it is used as a next try. Note, that in case once you have
   * returned with NULL, you might also want to resetWifiAuthInfo(), that sets
   * the auth info used for the next time to the one set up in the admin portal.
   * Note, that this feature is provided because of the option of providing
   * multiple WiFi settings utilized by the MultipleWifiAddition class. (See
   * IotWebConf2MultipleWifi.h)
   */
  ...
  /**
   * Add a custom parameter group, that will be handled by the IotWebConf2
   ...
   * before falling back to AP mode. The default amount can be updated with this
   * setter. Should be called before init()!
   */
  ...
  /**
   * Return the current state, that will be a value from the IOTWEBCONF_STATE_*
   * constants.
   */
  ...
  /**
   * This method can be used to set the AP timeout directly without modifying
   * the apTimeoutParameter. Note, that apTimeoutMs value will be reset to the
   * value of apTimeoutParameter on init and on config save.
   */
  ...
  /**
   * Returns the current WiFi authentication credentials. These are usually the
   * configured ones, but might be overwritten by
   * setWifiConnectionFailedHandler().
   */
  WifiAuthInfo getWifiAuthInfo() { return _wifiAuthInfo; };

  /**
   * Resets the authentication credentials for WiFi connection to the configured
   * one. With the return value of setWifiConnectionFailedHandler() one can
   * provide alternative connection settings, that can be reset with
   * resetWifiAuthInfo().
   */
  void resetWifiAuthInfo()
  {
    _wifiAuthInfo = {
        this->_wifiParameters._wifiSsid, this->_wifiParameters._wifiPassword};
  };

  /**
   * By default IotWebConf2 starts up in AP mode. Calling this method before the
   * init will force IotWebConf2 to connect immediately to the configured WiFi
   * network. Note, this method only takes effect, when WiFi mode is enabled,
   * thus when a valid WiFi connection is set up, and AP mode is not forced by
   * ConfigPin (see setConfigPin() for details).
   */
  void skipApStartup() { this->_skipApStartup = true; }

  /**
   * By default IotWebConf2 will continue startup in WiFi mode, when no
   * configuration request arrived in AP mode. With this method holding the AP
   * mode can be forced. Further more, instant AP mode can forced even when we
   * are currently in WiFi mode.
  ...
  /**
   * By default IotWebConf2 will set the thing password as the AP password when
   * connecting in AP mode With this method a default AP can be set to recover
   * settings. In combination with forceApMode, instan AP mode with default
   * password can be triggered
   *   @value - When parameter is TRUE the AP mode does not ask for password
   *     When value is FALSE normal operation will continue.
   */
  void forceDefaultPassword(bool value) { this->_forceDefaultPassword = value; }
...
private:
  ...
  NumberParameter _apTimeoutParameter = NumberParameter(
      ...
      IOTWEBCONF_WORD_LEN, IOTWEBCONF_DEFAULT_AP_MODE_TIMEOUT_SECS, NULL,
      "min='1' max='600'");
  ...
  std::function<bool(WebRequestWrapper* webRequestWrapper)> _formValidator =
      NULL;

...
void drawFrame3(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
void drawFrame4(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
void drawFrame5(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
void drawFrame6(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
void drawFrame7(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
void drawFrame8(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                int16_t y);
// void drawFrame9(OLEDDisplay *display, OLEDDisplayUiState* state, int16_t x,
// int16_t y);

uint8_t frameCount = 8;
FrameCallback frames[] = {drawFrame1, drawFrame8, drawFrame3, drawFrame5,
                          drawFrame2, drawFrame4, drawFrame6, drawFrame7};
...
void displayInit() {
  ...
  if (Wire.endTransmission() != 0) {
    LOG_CONSOLE(
        PSTR("OLED not found at 0x%02X (SDA=%d SCL=%d), display disabled."),
        board.OLED__address, board.OLED__SDA, board.OLED__SCL);
    return; // ui stays NULL → all displayXxx() calls gracefully skip
  }

  LOG_CONSOLE(
      PSTR("OLED detected at 0x%02X (SDA=%d SCL=%d RST=%d), initializing..."),
...
void msOverlay(OLEDDisplay *display, OLEDDisplayUiState *state) {
  ...
  char timeBuffer[12];
  snprintf(timeBuffer, sizeof(timeBuffer), "%2d:%02d:%02d", timeinfo->tm_hour,
  ...
  if (ConfigStore::getInstance().getDayNightOled()) {
    if (timeinfo->tm_hour < 6 || timeinfo->tm_hour > 18)
      ...
      display->invertDisplay(); // change the OLED according to the time.
  }
}

void drawRemoteFrame(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
                     int16_t y, uint8_t frameNumber) {
  if (status.remoteTextFrameLength[frameNumber] == 0)
    ui->nextFrame();
  ...
  for (uint8_t n = 0; n < status.remoteTextFrameLength[frameNumber]; n++) {
    ...
    display->drawString(x + status.remoteTextFrame[frameNumber][n].text_pos_x,
                        y + status.remoteTextFrame[frameNumber][n].text_pos_y,
...
void drawFrame3(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
  ...
  snprintf(displayBuffer, sizeof(displayBuffer), "%s @ %.4fMHz",
           status.modeminfo.modem_mode, status.modeminfo.frequency);
  ...
  if (strcmp(status.modeminfo.modem_mode, "LoRa") == 0) {
    snprintf(displayBuffer, sizeof(displayBuffer), "SF: %u",
             status.modeminfo.sf);
    display->drawString(x, 23 + y, displayBuffer);
    if (ConfigStore::getInstance().getAllowTx()) {
      snprintf(displayBuffer, sizeof(displayBuffer), "Pwr:%ddBm",
               status.modeminfo.power);
      display->drawString(x, 34 + y, displayBuffer);
    } else {
      display->drawString(x, 34 + y, "TX OFF");
    }
    display->setTextAlignment(TEXT_ALIGN_RIGHT);
    snprintf(displayBuffer, sizeof(displayBuffer), "BW:%.1fkHz",
             status.modeminfo.bw);
    display->drawString(128 + x, 23 + y, displayBuffer);
    snprintf(displayBuffer, sizeof(displayBuffer), "CR: %u",
             status.modeminfo.cr);
    display->drawString(128 + x, 34 + y, displayBuffer);
  } else {
    display->drawString(x, 23 + y, "FD/BW: ");
    if (ConfigStore::getInstance().getAllowTx()) {
      snprintf(displayBuffer, sizeof(displayBuffer), "P:%ddBm",
               status.modeminfo.power);
      display->drawString(x, 34 + y, displayBuffer);
    } else {
      display->drawString(x, 34 + y, "TX OFF");
    ...
    display->setTextAlignment(TEXT_ALIGN_RIGHT);
    snprintf(displayBuffer, sizeof(displayBuffer), "%.1f/%.1fkHz",
             status.modeminfo.freqDev, status.modeminfo.bw);
    display->drawString(128 + x, 23 + y, displayBuffer);
    snprintf(displayBuffer, sizeof(displayBuffer), "%.1fkbps",
             status.modeminfo.bitrate);
    display->drawString(128 + x, 34 + y, displayBuffer);
  }
}
...
void drawFrame8(OLEDDisplay *display, OLEDDisplayUiState *state, int16_t x,
  ...
  if (status.mqtt_connected) {
    display->drawString(x + 7, 26 + y, "ON");
  } else {
    display->drawString(x + 5, 26 + y, "OFF");
  }
  display->drawXbm(x + 32, y + 4, WiFi_Logo_width, WiFi_Logo_height,
                   WiFi_Logo_bits);
  // The coordinates define the center of the text
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(64 + x, 42 + y,
...
void displayShowConnected() {
  ...
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(
      64, 34, "WiFi:" + String(ConfigStore::getInstance().getWifiSSID()));
  display->drawString(64, 44,
                      "OTP --->    " + String(mqttCredentials.getOTPCode()));
...
void displayShowApMode() {
  ...
  display->drawString(0, 18,
                      "->" + String(ConfigStore::getInstance().getThingName()));
...
void displayShowStaMode(bool ap) {
  ...
  display->setTextAlignment(TEXT_ALIGN_CENTER);
  display->drawString(
      64, 35, "Connecting " + String(ConfigStore::getInstance().getWifiSSID()));
  if (ap)
 ...
 * If the brightness has changed, it sets the new brightness or turns off the
 * display. Also updates the UI if screen is on.
 */
void displayUpdate() {
  if (!ui)
    return;
